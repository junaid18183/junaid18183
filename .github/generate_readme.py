...
# Header
lines.append(f"# 👋 Hi, I'm {bio.get('name')}")
...
for job in work:
    lines.append(f"- **{job['position']}** at **{job['company']}** ({job['startDate']} – {job.get('endDate', 'Present')})")
    if job.get("summary"):
...
for certificate in certificates:
    lines.append(f"- **{certificate['name']}** from **{certificate['issuer']}** ({certificate['date']}) – {certificate.get('url', '')}")

lines.append("## 🚀 Projects")
for project in projects:
    lines.append(f"- [{project['name']}]({project['url']}) – {project['description']}")
